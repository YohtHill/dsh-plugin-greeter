...
export const GREETING_STYLES = ['minimal', 'warm', 'practical', 'engineering', 'playful', 'calm'];
...
export function readStore(file, warn = console.warn) {
    try {
 ...
 */
export function migrateLegacyStore(home, warn = console.warn) {
...
function pickTone(config) {
    if (config.style !== undefined)
        return STYLE_PROMPTS[config.style];
 ...
 */
export function greetingInstruction(userName, greetingIndex, config) {
...
export function apply(ctx, config) {
    ...
    const storeFileOf = (resolved) => join(resolveDshHome(), resolved.nameFile);
    // One-time migration from the legacy store name, at mount time (idempotent).
    if (effective().nameFile === 'greeter-store.json')
        migrateLegacyStore(resolveDshHome(), warn);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if (name === undefined)
                return 'That name looks empty — please ask the user for it again.';
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if (!GREETING_STYLES.includes(style)) {
    ...
    ctx.on('agent/session-start', ({ agent, source }) => {
        if (source !== 'startup' || !effective().proactive)
            return;
        if (greeted.has(agent))
            return;
        greeted.add(agent);
    ...
    ctx.on('agent/pre-step', async ({ agent, step }, next) => {
        ...
        if (decision.kind === 'reject' || step !== 1)
            return decision;
        if (greeted.has(agent))
            return decision;
        greeted.add(agent);
